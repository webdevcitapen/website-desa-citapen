...
export default function Berita() {
  ...
  useEffect(() => {
    const fetchBerita = async () => {
      try {
        setIsLoading(true);
        // Ambil semua data berita dengan paginasi (max 50 per halaman)
        let allDaftar: BeritaItem[] = [];
        let halamanFetch = 1;
        let totalHalaman = 1;
        do {
          const response = await api.get(`/berita?halaman=${halamanFetch}&perHalaman=50`);
          ...
          allDaftar = [...allDaftar, ...daftar];
          totalHalaman = tHalaman;
          halamanFetch++;
          // jika total belum diketahui, hentikan jika daftar kosong
          if (daftar.length === 0) break;
        } while (halamanFetch <= totalHalaman);
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    // Filter kategori (dummy: cek apakah judul/isi mengandung nama kategori, atau semua)
    if (kategoriFilter !== 'Semua') {
      const k = kategoriFilter.toLowerCase();
      result = result.filter(item =>
        item.judul.toLowerCase().includes(k) ||
        item.isi.toLowerCase().includes(k) ||
        (item.kategori && item.kategori.toLowerCase() === k)
      );
    }

    // Sort
    result.sort((a, b) => {
      const dateA = new Date(a.dibuatPada || a.dibuat_pada || 0).getTime();
      const dateB = new Date(b.dibuatPada || b.dibuat_pada || 0).getTime();
    ...
    return result;
  }, [berita, searchQuery, kategoriFilter, sortBy]);

  const featured = filteredAndSorted.slice(0, 3);
  const rest = filteredAndSorted.slice(3);
  const totalList = rest.length;
  const totalHalamanList = Math.max(1, Math.ceil(totalList / perHalaman));
  const halamanAktif = Math.min(halaman, totalHalamanList);
  const paginatedRest = rest.slice((halamanAktif - 1) * perHalaman, halamanAktif * perHalaman);
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full pt-12 pb-24">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-600 max-w-2xl text-sm md:text-base leading-relaxed">
            Dapatkan informasi terbaru mengenai program desa, kegiatan masyarakat, dan pengumuman penting di Desa Citapen.
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 md:p-5 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Cari berita (judul, isi)..."
                value={searchQuery}
          ...
          {(searchQuery || kategoriFilter !== 'Semua') && (
            <div className="mt-3 text-xs text-slate-500">
              Menampilkan {filteredAndSorted.length} dari {berita.length} berita
              {searchQuery && <> • Cari: <span className="font-bold text-[#0A3D2D]">"{searchQuery}"</span></>}
        ...
        {isLoading ? (
          ...
          <>
                  ...
                  {featured.map((item) => (
                    <div key={item.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col hover:shadow-md transition">
                      <div className="h-48 w-full overflow-hidden bg-slate-100">
                        <img src={getImageUrl(item.gambar)} alt={item.judul} className="w-full h-full object-cover" onError={(e) => { (e.currentTarget as HTMLImageElement).src = '/placeholder.svg'; }} />
                      </div>
                      <div className="p-5 flex flex-col flex-grow">
                        ...
                        <p className="text-slate-500 text-xs line-clamp-3 leading-relaxed flex-grow mb-3">
                          {item.isi}
                        </p>
                        <Link to={`/berita/${item.id}`} className="inline-flex items-center gap-1.5 text-[#0A3D2D] font-bold text-xs hover:underline mt-auto">
            ...
            {/* Daftar Berita List */}
            <div>
              <h2 className="text-lg font-bold text-[#0A3D2D] mb-4">Daftar Berita {rest.length > 0 && <span className="text-sm font-normal text-slate-500">({totalList} artikel {totalList > perHalaman ? `• Hal ${halamanAktif}/${totalHalamanList}` : ''})</span>}</h2>
              {paginatedRest.length === 0 ? (
                <div className="bg-white rounded-2xl border border-slate-200 p-8 text-center text-slate-500 text-sm">
                  Tidak ada berita lain. {featured.length > 0 ? 'Semua berita sudah tampil di 3 terbaru.' : ''}
                </div>
              ) : (
                <>
                    ...
                    {paginatedRest.map((item) => (
                      <div key={item.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col sm:flex-row hover:shadow-md transition">
                        ...
                        <div className="p-5 flex flex-col flex-grow">
                          ...
                          <Link to={`/berita/${item.id}`} className="inline-flex items-center gap-1.5 text-[#0A3D2D] font-bold text-xs hover:underline mt-3">
                            Baca Selengkapnya <ArrowRight className="w-3 h-3" />
                          </Link>
                        </div>
                      </div>
                  ...
                  {totalHalamanList > 1 && (
                    <div className="flex items-center justify-center gap-2 mt-8">
                      ...
                      {Array.from({ length: totalHalamanList }, (_, i) => i + 1).slice(Math.max(0, halamanAktif - 3), Math.min(totalHalamanList, halamanAktif + 2)).map(no => (
                        <button
                          key={no}
                          onClick={() => setHalaman(no)}
                          className={`w-9 h-9 rounded-xl font-bold text-sm ${no === halamanAktif ? 'bg-[#0A3D2D] text-white shadow' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                        >
                          {no}
                        </button>
                      ))}
                      <button
                        onClick={() => setHalaman(p => Math.min(totalHalamanList, p + 1))}
                        disabled={halamanAktif === totalHalamanList}
                        className="w-9 h-9 rounded-xl border border-slate-200 bg-white flex items-center justify-center hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed"
                      >
                  ...
                  <p className="text-center text-xs text-slate-400 mt-3">Halaman {halamanAktif} dari {totalHalamanList} • Total {totalList} berita di daftar</p>
                </>
              )}
            </div>
          </>
